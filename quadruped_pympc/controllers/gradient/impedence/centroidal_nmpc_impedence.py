...
from .centroidal_model_impedence import Centroidal_Model_Impedence
import numpy as np
import scipy.linalg
import casadi as cs
import copy
...
class Acados_NMPC_Impedence:
    def __init__(self):
        ...
        self.centroidal_model = Centroidal_Model_Impedence()
        ...
        self.acados_ocp_solver = AcadosOcpSolver(self.ocp,
                                                 json_file=self.ocp.code_export_directory + "/centroidal_nmpc_impedence" +
                                                           ".json")
    ...
    def create_stability_constraints(self, ) -> None:
        ...
        # only com inside the support polygon constraint
        x = 0.0
        y = 0.0
    ...
    def set_weight(self, nx, nu):
        ...
        if (config.robot == "hyqreal"):
            R_foot_force = np.array(
                [0.00001])  # f_x, f_y, f_z (should be 4 times this, once per foot)
            R_foot_impedence = np.array([0.0001, 0.0001])
        else:
            R_foot_force = np.array([0.001])
            R_foot_impedence = np.array([0.0001, 0.0001])
    ...
    def set_stage_constraint(self, constraint, state, reference, contact_sequence, h_R_w, stance_proximity):
        ...
        try:
            ...
            # We pass all the constraints to acados.
    ...
    def compute_control(self, state, reference, contact_sequence, constraint=None, external_wrenches=np.zeros((6,)),
        ...
        for j in range(self.horizon):
            ...
            yref[42] = reference_force_fl_z
            yref[43] = reference_force_fr_z
            yref[44] = reference_force_rl_z
            yref[45] = reference_force_rl_z
